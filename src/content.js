...
let plugSell = bagpack => {
    ...
    btn.on('click', () => {
        if (confirm('确认批量删除选中装备')) {
            let checboxs = bagpack.find('.equip-container > p input[type=checkbox]:checked')
            checboxs.each((i, n) => {
                let id = $(n).siblings('.equip-sell').data('id')
                $(n).siblings('.equip-name').css('text-decoration', 'line-through')
                sellEquipById(id, () => $(n).parents('.equip-container').remove())
            })
        }
    })
}
...
let plugStore = bagpack => {
    ...
    btn.on('click', () => {
        ...
        checboxs.each((i, n) => {
            let id = $(n).siblings('.equip-sell').data('id')
            storeEquipById(id, () => $(n).parents('.equip-container').appendTo($('.panel-inverse:eq(2) .panel-body')))
        })
    })
}
